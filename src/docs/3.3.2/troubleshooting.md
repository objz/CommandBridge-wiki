...
You used a platform-specific argument type in a script that registers on a location where that type is not available. For example, `PLAYERS` on Velocity requires PacketEvents; without it, validation fails.
...
**Fix:** Make sure the default value is valid for the argument type (for example a number for `INTEGER` or `DOUBLE`, or `true/false` for `BOOLEAN`).
