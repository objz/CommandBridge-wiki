...
1. Someone runs `/claim-reward <player>` on Velocity
...
4. If the target player is offline, the command is queued and runs when they reconnect
5. The `cooldown: 24h` prevents them from using it again for 24 hours (cooldowns are not persisted across restarts)
