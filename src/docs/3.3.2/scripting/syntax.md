...
Don't worry if this looks like a lot. It's not that complex. I just wanted to show everything in one place.
...
{% configentry "version", "integer", "4 | 3 | 2" %}
Schema version. Use `4`. Older versions are not loaded directly; migrate them first.
{% endconfigentry %}
...
The `server-id` or `client-id` of the target. Must match exactly what is in that instance's config. If it doesn't match, dispatch to that target fails at runtime.
{% endconfigentry %}
...
{% configentry "server", "object", "" %}
    ...
    If `true`, the command is skipped for targets where the resolved player is not currently online.
    {% endconfigentry %}
    ...
    {% configentry "player-arg", "string" %}
    Overrides which player identity is used for `target-required` and `schedule-online`.
    Set this to an argument name (not an argument type), for example `player`.
    The referenced argument must be player-resolvable (for example `STRING`, `PLAYERS`, or `OFFLINE_PLAYER`).
    {% endconfigentry %}

    {% hint "info" %}
    Scheduled commands are saved on the Velocity proxy at `plugins/commandbridge/data/tasks.json`.
    They survive restarts. If the script is deleted or renamed before the player connects, the queued task is discarded because there is nothing to execute anymore.
    {% endhint %}
{% endconfigentry %}
